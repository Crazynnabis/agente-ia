...
def detectar_zonas_sd(df: pd.DataFrame, sensibilidad: float = 0.015) -> dict:
    ...
    for i in range(2, len(df) - 2):
        vol_prom = np.mean(vols[max(0,i-10):i]) if i > 0 else 1

        if (closes[i] < opens[i] and closes[i+1] > opens[i+1] and
                closes[i+2] > closes[i+1] and vols[i] > vol_prom * 1.2):
            zonas_demanda.append({
                "precio_base": round(lows[i], 4),
                "precio_tope": round(closes[i], 4),
                "fuerza":      round(vols[i] / vol_prom, 2),
                "toques":      0,
            })
    ...
    if dist_soporte and dist_resistencia:
        ...
        if posicion_rango < 30:
            contexto = "CERCA_RESISTENCIA_precaucion"
