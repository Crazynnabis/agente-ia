...
CAPITAL_BASE   = 100000 # Fallback si no se puede leer Alpaca
...
def obtener_capital_real() -> float:
    """Lee el capital actual de Alpaca en lugar de usar valor hardcodeado."""
    try:
        ...
        print(f"[kelly] Error leyendo capital Alpaca: {e}")
    return CAPITAL_BASE
...
def calcular_tamaño_kelly(precio: float, stop_loss: float,
    ...
    # Capital real de Alpaca — no hardcodeado
...
def actualizar_parametros_kelly():
    ...
    print(f"[kelly] Capital actual: ${capital:,.2f}")
    ...
    print(f"[kelly] Riesgo por operacion: ${round(capital * kelly, 2)}")
